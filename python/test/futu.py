if __name__ == "__main__":
    ...
    sub_type_list = [ft.ft.SubType.K_1M, ft.SubType.K_5M, ft.SubType.K_15M, ft.SubType.K_30M, ft.SubType.K_60M,
                     ft.SubType.K_DAY, ft.SubType.K_WEEK, ft.SubType.K_MON]

    ret_status, ret_data = ft.quote_ctx.subscribe(stock_code_list, sub_type_list)
    if ret_status != ft.ft.RET_OK:
        print(ret_data)
        exit()

    ret_status, ret_data = ft.quote_ctx.query_subscription()
    ...
    for code in stock_code_list:
        for ktype in [ft.SubType.K_DAY, ft.SubType.K_1M, ft.SubType.K_5M]:
            ret_code, ret_data = ft.quote_ctx.get_cur_kline(code, 5, ktype)
...
def get_DF_RSI():
    ...
    ret, data = quote_ctx.get_cur_kline('US.AMZN', 1000, ft.SubType.K_60M, ft.AuType.QFQ)
    quote_ctx.close()
    if ret == ft.RET_OK:
...
def filterRSI():
    ...
    quote_ctx = ft.OpenQuoteContext(host='127.0.0.1', port=11111)
    quote_ctx.subscribe(['US.AMZN'], [ft.SubType.K_60M])
    ret, df = quote_ctx.get_cur_kline('US.AMZN', 1000, ft.SubType.K_60M, ft.AuType.QFQ)
    quote_ctx.close()
    if ret == ft.RET_OK:
